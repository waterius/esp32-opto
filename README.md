...
3. Адрес устройства в сети роутера — в списке клиентов роутера, в USB-логе или
   сканером [tools/lan_scan.html](tools/lan_scan.html): скачать файл, открыть в
   браузере, указать свою подсеть и нажать «Сканировать». Плата отвечает по HTTP
   на порту 80, по найденному адресу открывается её страница статуса.
