...
3. Адрес устройства в сети роутера — в списке клиентов роутера, в USB-логе или
   сканером [tools/lan_scan.html](tools/lan_scan.html): поднять
   `python3 -m http.server 8000 -d tools`, открыть
   `http://127.0.0.1:8000/lan_scan.html`, указать свою подсеть и нажать
   «Сканировать». Плата отвечает по HTTP на порту 80, по найденному адресу
   открывается её страница статуса. С телефона — тот же адрес, только вместо
   `127.0.0.1` адрес компьютера в сети. Открывать сканер прямо с диска
   (`file://`) не стоит: Chrome запрещает из него запросы, и «живыми» выглядят
   все адреса подряд.
