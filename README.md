...
Обе платы собираются под **4 МБ флеша** и раздел `min_spiffs.csv`. У S3
это задано явно: описание платы `esp32-s3-devkitc-1` обещает 8 МБ, а у
Super Mini на борту 4 (ESP32-S3FH4R2). Образ под 8 МБ в такую плату не
влезает, и **esptool при `flash_size=keep` границы не проверяет**: адреса
заворачиваются по кругу и затирают загрузчик в нуле — плата перестаёт
запускаться. Если у вас S3 с 8 или 16 МБ, размер и раздел правятся в
`platformio.ini`.
