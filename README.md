...
3. Адрес устройства в сети роутера — в списке клиентов роутера, в USB-логе или
   сканером [tools/lan_scan.html](tools/lan_scan.html): открыть файл в браузере,
   указать свою подсеть и нажать «Сканировать». Плата отвечает по HTTP на порту
   80, по найденному адресу открывается её страница статуса. Работает только
   локально: по `https://` браузер режет запросы на `http://192.168.x.x` как
   mixed content, поэтому на странице есть кнопка «Скачать файл». Если «живыми»
   выглядят все адреса подряд — запросы режет браузер, и файл надо отдать по
   http: `python3 -m http.server 8000 -d tools`.
4. На `/settings` задать ключ и e-mail облака Waterius.
