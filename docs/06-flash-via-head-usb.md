# 6. Прошивка платы через USB оптоголовки
...
| 3 | **R6** ↔ переходное отверстие | катод диода уходит на GPIO, драйвер головки в схеме не участвует |
| 4 | **R7** ↔ диод | анод отвязан от питания головки, диод питается с 3V3 платы ([02](02-rixutech-cp2102n-teardown.md#передатчик-ик-диод-напрямую-на-gpio)) |

Разрез «ножка 26 (TXD) ↔ правая площадка R2» из старой редакции **больше не
нужен**: после 3 и 4 диод отвязан от моста, бороться за линию нечему. Светодиод
не перепаивается — заводская полярность подходит.
...
| левая площадка **R6** | пин **5** платы | `OPTO_TX_PIN`, через R6 в катод диода |
| площадка диода со стороны **R7** | пин **3V3** платы | питание диода, 13,2 мА |
...
**Пин 21 оставить свободным**: на него выведен дубль лога (UART0, 115200), и
через USB-TTL лог читается, когда плата не на USB — у счётчика, в щитке, на
аккумуляторе. У S3 то же самое на GPIO43. Подробности —
[08-logging.md](08-logging.md#дубль-в-uart0).
...
| левая площадка **R6** | пин **12** платы | `OPTO_TX_PIN`, через R6 в катод диода |
| площадка диода со стороны **R7** | пин **3V3** платы | питание диода, 13,2 мА |
| одиночная ножка **V2** | пин **13** платы | `OPTO_RX_PIN`, с фототранзистора |
...
- **Набор выведенных пинов у клонов разный**: где-то это IO1–IO18 и IO21,
  ...
  шелкографией]**. Поэтому взяты 12 и 13: они есть у обоих вариантов и стоят
  рядом с 3V3, GND и 5V, так что пять проводов паяются одной группой. Если и их
  нет, брать любые свободные и править `OPTO_RX_PIN` / `OPTO_TX_PIN` в
  `platformio.ini` — больше в коде менять нечего.
- Доработка **закрывает вариант «недоработанная головка через USB-хост»** из
  [04](04-alternatives.md): порт S3 становится устройством.
