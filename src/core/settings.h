...
// Поднимать ТОЛЬКО когда меняется смысл уже существующего поля: единицы,
// интерпретация, имя, разделение на два. Добавление и удаление поля версию не
// двигают — запись в NVS самоописательная (JSON, core/settings_io.*), новое
// поле берёт умолчание само, удалённое молча игнорируется.
//
// На каждый подъём — функция апгрейда в settings_io.cpp и тест на неё
// (test/test_settings). Иначе обновление прошивки снова начнёт стоить человеку
// пароля Wi-Fi и ключа облака.
const uint16_t SETTINGS_VERSION = 4;

// Поля версии в структуре нет: в памяти версия всегда текущая, а в записи за
// неё отвечает ключ "v" (core/settings_io.*).
struct Settings {
    // Wi-Fi
    char ssid[33] = "";
    char pass[65] = "";
