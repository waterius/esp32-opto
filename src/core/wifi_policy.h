...
struct WifiPolicyCfg {
    ...
    // Пока на точке доступа кто-то сидит, попытки подключения приостановлены:
    // радио одно, и каждая попытка обрывает ему сессию настройки. Но не
    // навсегда — забытый на точке телефон иначе оставит устройство офлайн
    // насовсем. 0 — не приостанавливать.
    uint32_t apBusyHoldMs = 180000;
};
...
class WifiPolicy {
   ...
   private:
    ...
    bool apBusyWas_ = false;         // клиент точки был на прошлом шаге
    uint32_t apBusySinceMs_ = 0;     // когда он появился
};
