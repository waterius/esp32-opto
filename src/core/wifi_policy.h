...
struct WifiPolicyCfg {
    ...
    // Столько точка доступа живёт после подключения, начатого со страницы, даже
    // если на ней никого не осталось. Радио одно: подключаясь к роутеру, оно
    // уходит на его канал, точка переезжает следом и сбивает с себя телефон
    // (espressif/esp-idf#8553). Без этой выдержки «на точке никого» наступает
    // через доли секунды после успеха, точку гасят — и человеку некуда
    // вернуться за новым адресом устройства. 0 — не держать.
    uint32_t apHoldAfterConnectMs = 180000;
};
...
class WifiPolicy {
   public:
    ...
    // Человек подключает устройство к сети со страницы портала: точку доступа
    // после успеха придержать, чтобы он мог вернуться на неё и прочитать
    // полученный адрес. Отсчёт пойдёт не отсюда, а с момента, когда связь
    // действительно появится, — именно тогда телефон и теряет точку.
    void requestApHold() { apHoldPending_ = true; }
   ...
   private:
    ...
    bool apHoldPending_ = false;     // подключение начато со страницы, выдержка ещё не пошла
    bool apHoldRunning_ = false;     // выдержка идёт: точку не гасим даже без клиентов
    uint32_t apHoldStartMs_ = 0;     // когда связь появилась и телефон потерял точку
