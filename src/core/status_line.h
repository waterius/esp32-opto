...
struct StatusFacts {
    ...
    uint8_t apChannel = 0;     // рабочий канал радио; 0 — точки нет
    // Канал из конфига точки. Отличается от apChannel — значит точка «поднята»,
    // а маяков в эфире нет: радио работает не на том канале, что записан в её
    // конфиге. Ровно этот случай стоил дня поисков несуществующей сети.
    uint8_t apCfgChannel = 0;
    uint8_t apClients = 0;     // сколько клиентов сидит на точке
