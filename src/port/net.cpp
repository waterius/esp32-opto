...
// на канале 1, радио ушло на 6, в эфире сети не было (см. docs/07-reliability).
...
void reconnect(const core::Settings& s) {
    ...
    // Сеть прислали со страницы, значит человек сейчас на точке доступа и
    // вот-вот её потеряет: радио уйдёт на канал роутера. Точку после успеха
    // держим ещё несколько минут, чтобы он вернулся на неё и прочитал адрес.
    policy.requestApHold();
}
