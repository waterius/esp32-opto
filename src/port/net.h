...
// Связь есть и подтверждена делом: драйвер сообщает о подключении, и пакеты
// действительно ходят. Мёртвый линк, который драйвер считает живым, для всей
// прошивки выглядит как отсутствие связи.
bool connected();

// Отдельно — вердикт контроля живости, для страницы статуса.
bool linkAlive();

// Контроль живости взведён: хотя бы одна проверка связи когда-то прошла.
// Пока false, связь мёртвой он не объявит — это предохранитель для сетей,
